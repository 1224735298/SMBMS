...
<div class="right">
       ...
       <div class="search">
       <form method="post" action="${pageContext.request.contextPath }/BillServlet">
		   ...
		   <c:if test="${User.userRole==3}">
			   <a  href="${pageContext.request.contextPath }/BillServlet?method=query" >刷新订单</a>
		   </c:if>
      ...
      <table class="providerTable" cellpadding="0" cellspacing="0">
          ...
          <c:forEach var="bill" items="${pager.datas }" varStatus="status">
				<tr>
					<td>
					...
					<c:if test="${User.userRole==1 or User.userRole==2}">
						<span><a class="modifyBill" href="${pageContext.request.contextPath }/BillServlet?method=toModify&id=${bill.id}" billid=${bill.id } billcc=${bill.billCode }><img src="${pageContext.request.contextPath }/images/xiugai.png" alt="修改" title="修改"/></a></span>
					</c:if>
					<c:if test="${User.userRole==1 or User.userRole==2}">
						<span><a class="deleteBill" href="javascript:void(0)" billid=${bill.id } billcc=${bill.billCode }><img src="${pageContext.request.contextPath }/images/schu.png" alt="删除" title="删除"/></a></span>
					</c:if>
